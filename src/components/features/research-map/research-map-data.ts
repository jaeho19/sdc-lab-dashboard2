...
export type NodeType =
  ...
  | "theme"        // L1
  | "rq"           // L2 ★v2 신규
  | "method" | "tech" | "dataset"  // L3 (dataset ★v2 신규)
  | "student" | "project" | "course"  // L4 (course ★v2 신규, 이번 라운드 미사용)
  | "paper" | "conference";        // L5

export type Layer = 0 | 1 | 2 | 3 | 4 | 5;

export const LAYER_OF: Record<NodeType, Layer> = {
  axis: 0, theme: 1, rq: 2,
  method: 3, tech: 3, dataset: 3,
  student: 4, project: 4, course: 4,
  paper: 5, conference: 5,
};

export type AxisId = "urban" | "rural" | "bridge";

// ─── 엣지 타입 (10종 v2 + v1 backward-compat) ───
/** v2 온톨로지 엣지 10종 */
export type LinkTypeV2 =
  | "advisory"      // 학생 → topic/rq (지도관계)
  | "method-use"    // topic/student → method/tech
  | "collab-pair"   // 학생 ↔ 학생
  | "authors"       // 학생 → paper
  | "presents-at"   // topic/student/paper → conference
  | "is-a"          // 하위 → 상위 (온톨로지 종 관계)
  | "part-of"       // 부분 → 전체 (구성 관계)
  | "uses-dataset"  // 학생/topic → dataset
  | "teaches"       // course → topic  (v2 대비, 이번 라운드 미사용)
  | "funded-by";    // project → funder (v2 대비, 이번 라운드 미사용)

/** v1 호환 별칭 — Phase 2 마이그레이션 완료 후 제거 예정.
 *  - link → advisory(학생→theme) 또는 method-use(학생→method/tech)
 *  - collab → collab-pair
 *  - kk → similarTo/is-a/part-of (노드 parents·similarTo 필드로 대체) */
export type LinkTypeV1 = "link" | "collab" | "kk";

export type LinkType = LinkTypeV2 | LinkTypeV1;
...
export interface MapNode {
  ...
  // ─── v2 신규: 온톨로지 레이어 ───
  /** 온톨로지 레이어 (LAYER_OF[type]에서 자동 추론 가능하지만 명시) */
  layer?: Layer;
  /** is-a: 상위 개념 ID (예: k_vlm → k_ai). 보통 같은 L에서 L-1 방향 */
  parents?: string[];
  /** similar-to: 같은 레이어 유사 개념 ID (대칭 관계, 양쪽에 모두 기재) */
  similarTo?: string[];
  /** 타임라인 뷰용 (L4 student, L5 paper) */
  ...
  rq?: {
    question: string;
    hypothesis?: string;
    status: "open" | "active" | "answered";
    owner: string;       // 학생 id
    techs?: string[];    // L3 tech 노드 id 참조 (parents는 L1 themes만)
  };
  /** course 전용 (이번 라운드 미사용, v2 대비) */
  course?: {
    year: number;
    level: "ug" | "gr" | "pro";
    credits?: number;
...
export interface MapLink {
  source: string;
  target: string;
  /** v2: 10종 중 하나. v1 "link"|"collab"|"kk"는 v2에서 각각
   *  advisory/method-use(link), collab-pair(collab), is-a/part-of/similar(kk)로 대체 */
  type: LinkType;
  /** 시각적 굵기 (1=얇음, 5=매우 굵음). 기본 2 */
  weight?: 1 | 2 | 3 | 4 | 5;
  /** 호버 시 표시 */
  label?: string;
}
...
export const NODE_COLORS: Record<NodeType, string> = {
  axis: "#e8e070",
  theme: "#7ec884",
  rq: "#b57edc",          // L2 ★v2 신규 (보라)
  method: "#c084e0",
  tech: "#50d0d0",
  dataset: "#3ba9d4",     // L3 ★v2 신규 (청록)
  student: "#5bc0eb",
  project: "#f0a050",
  course: "#d4c04e",      // L4 ★v2 신규 (머스터드, 미사용)
  paper: "#f5a623",
  conference: "#e05858",
};
...
// ─── 빌드 타임 검증 ───
/** 모든 링크의 source/target이 노드 id에 존재하는가 검증. NODES/LINKS export 후 호출. */
export function validateGraph(nodes: MapNode[], links: MapLink[]): void {
  const ids = new Set(nodes.map(n => n.id));
  for (const l of links) {
    if (!ids.has(l.source)) throw new Error(`[research-map] Invalid link source: ${l.source}`);
    if (!ids.has(l.target)) throw new Error(`[research-map] Invalid link target: ${l.target}`);
  }
  // 이지윤 참조 0건 검증 (G20)
  const jiyoonRefs = [
    ...nodes.filter(n => n.id === "이지윤").map(n => `node:${n.id}`),
    ...nodes.flatMap(n => (n.parents ?? []).includes("이지윤") ? [`parents:${n.id}`] : []),
    ...nodes.flatMap(n => (n.similarTo ?? []).includes("이지윤") ? [`similarTo:${n.id}`] : []),
    ...links.filter(l => l.source === "이지윤" || l.target === "이지윤").map(l => `link:${l.source}→${l.target}`),
  ];
  if (jiyoonRefs.length > 0) {
    throw new Error(`[research-map] 이지윤 참조 발견 (G20 위반): ${jiyoonRefs.join(", ")}`);
  }
}
...
export const NODES: MapNode[] = [
  // ────────────── 연구축 (L0) ──────────────
  {
    id: "urban", label: "도시", type: "axis", layer: 0, size: 44, axes: ["urban"],
    desc: "공원·녹지·보행환경 연구축",
    body: "<p>공원 접근성, 녹지 형평성, 보행환경 쾌적성, GSV+AI 분석</p><p><strong>학생:</strong> 이다연, 김가인, 김주연</p>",
    actions: [{ title: "CELA/AAG 2027", desc: "도시 주제 발표 학회" }],
  },
  {
    id: "rural", label: "농촌", type: "axis", layer: 0, size: 44, axes: ["rural"],
  ...
  // ────────────── 학생 (L4) ──────────────
  {
    id: "이다연", label: "이다연", type: "student", layer: 4, size: 22, axes: ["urban"], avatar: "/images/lee-dayeon.png", timeRange: { start: "2024" },
  ...
  {
    id: "김은솔", label: "김은솔", type: "student", layer: 4, size: 22, axes: ["rural"], avatar: "/images/kim-eunsol.png", timeRange: { start: "2023" },
  ...
  {
    id: "이은진", label: "이은진", type: "student", layer: 4, size: 22, axes: ["rural"], avatar: "/images/lee-eunjin.png", timeRange: { start: "2024" },
  ...
  {
    id: "최희진", label: "최희진", type: "student", layer: 4, size: 22, axes: ["rural"], avatar: "/images/choi-heejin.png", timeRange: { start: "2024" },
  ...
  {
    id: "배성훈", label: "배성훈", type: "student", layer: 4, size: 22, axes: ["rural"], avatar: "/images/bae-seonghun.png", timeRange: { start: "2024" },
  ...
  {
    id: "김가인", label: "김가인", type: "student", layer: 4, size: 22, axes: ["urban"], avatar: "/images/kim-gain.jpg", timeRange: { start: "2024" },
  ...
  {
    id: "김주연", label: "김주연", type: "student", layer: 4, size: 22, axes: ["urban"], avatar: "/images/kim-juhyeon.png", timeRange: { start: "2025" },
  ...
  // ────────────── 프로젝트 (L4) ──────────────
  {
    id: "nrf", label: "신진연구 NRF", type: "project", layer: 4, size: 24, axes: ["urban", "rural", "bridge"], timeRange: { start: "2025", end: "2028" },
  ...
  {
    id: "forest", label: "산림청", type: "project", layer: 4, size: 24, axes: ["urban"], timeRange: { start: "2025" },
  ...
  // ────────────── 학회 (L5) ──────────────
  {
    id: "cela", label: "CELA", type: "conference", layer: 5, size: 22, timeRange: { start: "2027-03" },
    desc: "조경학 | 2027: ~3월, 초록 ~9/2026",
    body: "<p>2026 완료. 2027 초록 9월 예상.</p><p>적합: 녹지, 형평성, 쾌적성, 보행환경</p>",
    actions: [{ title: "2027 초록(~9월)", desc: "김가인, 김주연" }],
  },
  {
    id: "aag", label: "AAG", type: "conference", layer: 5, size: 22, timeRange: { start: "2027-02" },
    desc: "공간/지리 다학제 | 2027: 2/8-12 뉴욕",
    body: "<p>2027 뉴욕, 초록 10-11월.</p><p>적합: AI, 공간분석, 원격탐사, 환경정의</p>",
    actions: [{ title: "2027 초록(~10월)", desc: "이다연, 김은솔, 배성훈" }],
  },
  {
    id: "acsp", label: "ACSP", type: "conference", layer: 5, size: 22, timeRange: { start: "2026-10" },
  ...
  // ────────────── 주제 키워드 (theme, L1) ──────────────
  {
    id: "k_지표개발", label: "지표개발", type: "theme", layer: 1, size: 15,
    parents: ["urban", "rural"], similarTo: ["k_형평성"], axes: ["urban", "rural", "bridge"],
    ...
    <li><strong>김은솔:</strong> EWI 조기경보</li>
    <li><strong>신진연구:</strong> 통합 평가 지표</li></ul>`,
  },
  {
    id: "k_형평성", label: "형평성", type: "theme", layer: 1, size: 14,
    parents: ["urban", "rural"], similarTo: ["k_접근성", "k_취약성", "k_기후"], axes: ["urban", "rural", "bridge"],
    desc: "Equity — 녹지·기후·에너지",
    body: "<ul><li>산림청: 형평적 배분</li><li>신진연구: 형평성 정책</li><li>김주연: G2SFCA+LISA</li><li>김은솔: 에너지 정의</li></ul>",
  },
  {
    id: "k_접근성", label: "접근성", type: "theme", layer: 1, size: 14,
    parents: ["urban", "rural"], similarTo: ["k_형평성", "k_녹지"], axes: ["urban", "rural"],
    desc: "Accessibility — 녹지·인프라·서비스",
    body: "<ul><li>김주연: G2SFCA·전국 지도화</li><li>산림청: 300m 접근</li><li>배성훈: 인프라 접근시간</li></ul>",
  },
  {
    id: "k_취약성", label: "취약성", type: "theme", layer: 1, size: 13,
    parents: ["urban", "rural"], similarTo: ["k_형평성", "k_지표개발"], axes: ["urban", "rural"],
    desc: "Vulnerability — 농업·기후·사회경제",
    body: "<ul><li>김은솔: 농업기반 취약성</li><li>산림청: 기후·사회경제 6지표</li><li>최희진: 공동체 위기</li></ul>",
  },
  {
    id: "k_녹지", label: "녹지/공원", type: "theme", layer: 1, size: 15,
    parents: ["urban"], similarTo: ["k_접근성", "k_보행", "k_기후"], axes: ["urban"],
    desc: "Green Space — 공원·비공원·도시숲",
    body: "<ul><li>김가인: DPT 녹지</li><li>김주연: K-TES·66,834 녹지</li><li>산림청: 도시숲</li><li>신진연구: 통합 평가</li></ul>",
  },
  {
    id: "k_보행", label: "보행환경", type: "theme", layer: 1, size: 13,
    parents: ["urban"], similarTo: ["k_녹지", "k_웰빙"], axes: ["urban"],
    desc: "Walkability",
    body: "<ul><li>이다연: VLM 보행환경</li><li>김가인: DPT 쾌적성</li></ul>",
  },
  {
    id: "k_수용성", label: "수용성/참여", type: "theme", layer: 1, size: 13,
    parents: ["rural"], similarTo: ["k_갈등", "k_공동체"], axes: ["rural"],
    desc: "Acceptance / Participation",
    body: "<ul><li>이은진: LLM 정책수용성</li><li>김은솔: 보완형 vs 대체형</li><li>최희진: 참여·역량</li></ul>",
  },
  {
    id: "k_갈등", label: "갈등", type: "theme", layer: 1, size: 12,
    parents: ["rural"], similarTo: ["k_수용성", "k_공동체"], axes: ["rural"],
    desc: "Conflict — 공동체·이해관계자",
    body: "<ul><li>최희진: 갈등 구조화·지표</li><li>이은진: 이해관계자 갈등 탐색</li></ul>",
  },
  {
    id: "k_공동체", label: "공동체", type: "theme", layer: 1, size: 13,
    parents: ["rural"], similarTo: ["k_갈등", "k_수용성"], axes: ["rural"],
    desc: "Community — 마을, 주민관계",
    body: "<ul><li>최희진: 마을공동체 갈등</li><li>이은진: 농촌 이해관계자</li><li>배성훈: 이웃관계·결속</li></ul>",
  },
  {
    id: "k_유형", label: "공간유형분류", type: "theme", layer: 1, size: 13,
    parents: ["urban", "rural"], similarTo: ["k_정책"], axes: ["urban", "rural", "bridge"],
    desc: "Spatial Typology — 유형화→차별적 정책",
    body: "<ul><li>배성훈: 법정리 도농복합</li><li>김은솔: 농업기반 유형</li><li>산림청: 구도심 6유형</li></ul>",
  },
  {
    id: "k_정책", label: "정책지원", type: "theme", layer: 1, size: 13,
    parents: ["urban", "rural"], similarTo: ["k_유형", "k_지표개발"], axes: ["urban", "rural", "bridge"],
    desc: "Policy — 대시보드, 시뮬레이션",
    body: "<ul><li>신진연구: GIS 대시보드</li><li>산림청: 7단계 로드맵</li><li>이은진: 사전탐색 도구</li><li>김은솔: 허용조건</li></ul>",
  },
  {
    id: "k_기후", label: "기후정의", type: "theme", layer: 1, size: 12,
    parents: ["urban", "rural"], similarTo: ["k_형평성", "k_녹지"], axes: ["urban", "rural", "bridge"],
    desc: "Climate Justice",
    body: "<ul><li>산림청: 기후취약성</li><li>신진연구: 형평성×기후 연동</li></ul>",
  },
  {
    id: "k_웰빙", label: "웰빙/만족", type: "theme", layer: 1, size: 12,
    parents: ["urban"], similarTo: ["k_보행"], axes: ["urban", "rural"],
    desc: "Well-being — 삶의 질, 쾌적성",
    body: "<ul><li>배성훈: 정주만족도</li><li>김가인: 보행 쾌적성</li></ul>",
  },
  {
    id: "k_에너지", label: "에너지전환", type: "theme", layer: 1, size: 11,
    parents: ["rural"], similarTo: ["k_수용성"], axes: ["rural"],
  ...
  // ────────────── 방법론 키워드 (method, L3) ──────────────
  {
    id: "k_설문", label: "설문/조사", type: "method", layer: 3, size: 14,
    similarTo: ["k_llm"],
    desc: "★ 도시↔농촌 방법론 브릿지",
    body: "<ul><li>최희진: CCDI+EFA</li><li>김은솔: 450명+LPA</li><li>이은진: LLM 설문대체</li><li>배성훈: 2,645명</li><li>김가인: 100명+ 쾌적성</li></ul>",
  },
  {
    id: "k_gis", label: "GIS/공간분석", type: "method", layer: 3, size: 14,
    similarTo: ["k_rs"],
    desc: "ArcGIS, LISA, PCA, HLM",
    body: "<ul><li>김주연: ArcGIS·LISA</li><li>배성훈: PCA, HLM</li><li>김은솔: Moran, FLUS</li></ul>",
  },
  {
    id: "k_rs", label: "원격탐사", type: "method", layer: 3, size: 13,
    similarTo: ["k_gis"],
  ...
  // ────────────── 기술 키워드 (tech, L3) ──────────────
  {
    id: "k_ai", label: "AI/딥러닝", type: "tech", layer: 3, size: 14,
  ...
  {
    id: "k_gsv", label: "Street View", type: "tech", layer: 3, size: 12,
    similarTo: ["k_cv"],
    desc: "Google/Naver SV 이미지",
    body: "<ul><li>이다연: 8,263 GSV</li><li>김가인: 송파/종로 GSV</li><li>산림청: 녹시율</li></ul>",
  },
  {
    id: "k_cv", label: "CV/Seg", type: "tech", layer: 3, size: 12,
    parents: ["k_ai"], similarTo: ["k_vlm", "k_dpt"],
    desc: "DeepLabV3+, MaskFormer, DETR",
    body: "<ul><li>이다연: DeepLabV3+ 19클래스</li><li>김가인: MaskFormer 150범주</li></ul>",
  },
  {
    id: "k_dpt", label: "DPT/3D", type: "tech", layer: 3, size: 11,
    parents: ["k_ai", "k_cv"],
    desc: "Dense Prediction Transformer",
    body: "<p>2D→3D 깊이 추정 → 녹지 인식 거리</p><p>사용: 김가인</p>",
  },
  {
    id: "k_vlm", label: "VLM", type: "tech", layer: 3, size: 12,
    parents: ["k_ai"], similarTo: ["k_llm", "k_cv"],
    desc: "Vision-Language Model (GPT-4o)",
    body: "<p>이미지→텍스트 평가 + 설명 생성</p><p>사용: 이다연</p>",
  },
  {
    id: "k_llm", label: "LLM", type: "tech", layer: 3, size: 12,
    parents: ["k_ai"], similarTo: ["k_vlm"],
